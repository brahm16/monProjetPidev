...
public class AddFournisseurFXMLController implements Initializable {
    ...
    public void handleAdd(ActionEvent event){
        ...
        if((verifEmail(txtEmail.getText()))&&(validatePhoneNumber(txtPhoneNumber.getText()))&&(txtFirstName.getText().length()>3)&&(txtLastName.getText().length()>3)&&(txtAddress.getText().length()>8)){
            Fournisseur f=new Fournisseur(txtFirstName.getText(),txtLastName.getText(),txtPhoneNumber.getText(),cmbTypeProduct.getValue().toString(),txtAddress.getText(),txtEmail.getText());
